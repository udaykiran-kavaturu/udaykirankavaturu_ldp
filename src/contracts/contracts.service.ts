...
export class ContractsService {
  ...
  async createContract(createContractDTO: CreateContractDTO, currentUserID: number) {
    const newContract =
  ...
  async updateContract(id: number, contract: Partial<Contract>, currentUserID: number, currentUserType: string) {
    const contractDetails = await this.contractsRepository.findOne({ where: { id } });
    if (!contractDetails)
      throw new HttpException('contract not found', HttpStatus.NOT_FOUND);

    if (contractDetails.lender_id != currentUserID && currentUserType != UserType.ADMIN)
      throw new ForbiddenException('You can only view or update your own contract');
